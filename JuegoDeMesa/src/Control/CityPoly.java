...
public class CityPoly implements IConstants {
    ...
    public CityPoly() {
        ...
        DecC = build.buildCityDeck();
        ArchivoSecuencial rankingread = new ArchivoSecuencial();
        try {
            Ranking = rankingread.leerSecuencialRanking();
    ...
    public void SaveTree() throws IOException {
        ...
        for (Jugador player : PlayersInNode) {
            save.escribirSecuencialUsuarios(player);
        }
    ...
    public void SaveRanking() throws IOException {
        ...
        while (index != Ranking.size()) {
            ...
            save.escribirSecuencialRanking(Ranking.get(index).getName(), Integer.toString(Ranking.get(index).getPointsLife()));
            index++;
        }
    }
     ...
     * en la ciudad y va armando el grafo segun sus distancias, contando como
        ...
        while (indexP < City.getPlaces().size() - 1) {
            ...
            while (indexP2 < City.getPlaces().size() - 1) {
                ...
                if (!(indexP == indexP2)) {
                    ...
                    distanciaEntrePlace = Tools.distance(P.getLatitud(), P.getLongitud(), P2.getLatitud(),
                            P2.getLongitud());
    ...
    private int searchNear(int CurrentPos, Type TipoBuscado) {
        ...
        for (Place lugar : City.getPlaces()) {
            ...
            if (lugar.getTipo().equals(TipoBuscado)) {
                DistanciActual = (int) Tools.distance(
    ...
    public ArrayList<CypherExportable> getCypherPlaces(){
        ArrayList<CypherExportable> ArrayCypher = new ArrayList();
        for (Place lugar : this.City.getPlaces()){
            ArrayCypher.add(lugar);
        }
        return ArrayCypher;
    }
