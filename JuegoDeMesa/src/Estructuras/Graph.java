...
public class Graph {
    ...
    private List< List< Node > > AdyacencyList = new ArrayList< List< Node > >(); //lista de adyacencia
    ...
    public Graph(int V) {
        ...
        this.CantVertices = V;
        for( int i = 0 ; i <= V ; ++i ) 
            AdyacencyList.add(new ArrayList<Node>()) ; //inicializamos lista de adyacencia
    ...
    void addEdge( int origen , int destino , int peso , boolean dirigido ){
        AdyacencyList.get( origen ).add( new Node( destino , peso ) );    //grafo diridigo
        if( !dirigido )
            AdyacencyList.get( destino ).add( new Node( origen , peso ) ); //no dirigido
    }
    ...
    public List<List<Node>> getAdyacencyList() {
        return AdyacencyList;
    }

    public void setAdyacencyList(List<List<Node>> AdyacencyList) {
